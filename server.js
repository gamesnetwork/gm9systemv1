...
const youtube = new YouTube("AIzaSyDi1av6S3vJGADNdnahEyOpV97pDfM-Bac"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyDi1av6S3vJGADNdnahEyOpV97pDfM-Bac"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("ready", () => {
  ...
  client.user.setActivity(`${prefix}help`, { type: "GN9 SB" });
});
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@571543926264037378>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
\`أوامر الصوت \` :notes:
\`${prefix}Play\` : تشغيل مقطع صوتي او اضافتها للقائمة او اكمال مقطع صوتي [p]
\`${prefix}Pause\` : ايقاف مؤقت مقطع صوتي
\`${prefix}Resume\` : اكمال مقطع صوتي 
\`${prefix}stop\` : لأيقاف مقطع صوتي وخروج البوت من الروم
\`${prefix}forceskip\` : لتخطي مقطع صوتي بشكل مباشر
\`${prefix}Queue\` : عرض القائمة 
\`${prefix}skipto\` : لتخطي مقطع صوتي الى مقطع صوتي القادمة في طابور الموسيقى القادمة
\`${prefix}Skip\` : تخطي لمقطع صوتي التالية 
\`${prefix}Volume\` : تغيير الصوت [vol] 
\`${prefix}np\` : عرض مايتم تشغيله الان [np] 
\`${prefix}repeat\` : تكرار المقطع الصوتي 
...
client.on("message", message => {
  if (message.content === "هلو") {
    message.channel.send("**:heart:هلوات:heart:**");
    message.channel.sendFile("");
  }
});
